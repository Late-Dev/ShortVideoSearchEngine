<template>
  <v-container>
    <v-text-field
      ...
      density="compact"
      color="primary"
      @input="debouncedGetSearch"
    ></v-text-field>
    <v-virtual-scroll item-height="'calc(50vh)'" :height="'calc(100vh - 45px)'"
 :items="videos" >
      <template  v-slot:default="{ item }">
        <div style="display: flex; position: relative; justify-content: center;">
          ...
          <div style=" position: absolute; bottom: 80px; text-align: center; background: rgba(0, 0, 0, 0.5); border-radius: 10px">{{ item.description }}</div>
